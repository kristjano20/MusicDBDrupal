...
class MusicSearchDataHelper {
   ...
   */
  public static function combineResults(array $spotify_results, array $discogs_results, string $type = 'artist'): array {
    $combined_data = [];
    $seen_names = [];
    ...
    foreach ($discogs_results as $item) {
      ...
      $normalized = self::normalizeName($title);
      $found = FALSE;
      foreach ($combined_data as &$existing) {
        if (self::normalizeName($existing['name']) === $normalized) {
          $existing['discogs_id'] = $item['id'] ?? '';
          $found = TRUE;
          break;
        }
      }
      if (!$found && !isset($seen_names[$normalized])) {
        $combined_data[] = [
          'name' => $title,
          'spotify_id' => '',
          'discogs_id' => $item['id'] ?? '',
